...
# Reading the CSV files that belong to the test group (EyeT_group_dataset_III_*.csv) from the dataset directory, appending them all and saving them as test_csv.csv. Here we make sure to check if a file named tset_csv.csv, exists in the directory before creating a new file.
...
# Get a list of all the files starting with 'EyeT_group_dataset_III_' in the directory
file_list = [f for f in os.listdir(path) if f.startswith('EyeT_group_dataset_III_')]
...
test_csv = pd.concat(dfs, ignore_index=True)
...
if os.path.exists(filename):
    ...
    test_csv.to_csv(filename, index=False)
    print(f"The file {filename} has been saved.")


# Reading the the first 5 rows of a dataframe test_csv
...
# In[8]:
all_participants_test = feature_selection(test_csv)
print(all_participants_test)
...
corr = all_participants_test[['Label', 'Gaze event duration']].corr()
print(corr)


# Invoking the add_empathy_scores() function for the test group dataset and saving the output as test_dataset

# In[140]:


test_dataset = add_empathy_scores(all_participants_test)


# <h2>Pre-processing data</h2>
# Reshaping the test_dataset array to a 2-dimensional array

# In[141]:


test_dataset = np.reshape(test_dataset, (-1, 8))
test_dataset = pd.DataFrame(test_dataset, columns=['Participant name', 'Recording timestamp', 'Label', 'Gaze event duration', 'empathy_before_std', 'empathy_before_ext', 'empathy_after_std', 'empathy_after_ext'])


# Filling the missing values in test_dataset using the fillna() method with the mean of each column 

# In[142]:


test_dataset = test_dataset.fillna(df.mean())
nan_counts = test_dataset.isna().sum()
print(nan_counts)
...
mean_empathy_before_std = test_dataset['empathy_before_std'].mean()
print(mean_empathy_before_std)

# Create a new column empathy_level_before_std based on mean value
test_dataset['empathy_level_before_std'] = np.where(test_dataset['empathy_before_std'] >= mean_empathy_before_std, 'High', 'Low')
...
mean_empathy_after_std = test_dataset['empathy_after_std'].mean()
print(mean_empathy_after_std)

# Create a new column empathy level_after_std based on mean value
test_dataset['empathy_level_after_std'] = np.where(test_dataset['empathy_after_std'] >= mean_empathy_after_std, 'High', 'Low')


# Printing the columns names and values saved in test_dataset and saving the content to a file called 'test_dataset.csv' to further inspections (To check if the new columns are saved with correct values).
...
test_dataset.to_csv(filename, index=False)
...
# Calculate correlation between Gaze event duration and Eye movement type
correlation_matrix = test_dataset[['Label', 'Gaze event duration', 'empathy_before_std', 'empathy_before_ext', 'empathy_after_std', 'empathy_after_ext']].corr()
print(correlation_matrix)


# <h2>Splitting dataset for training, validation, and testing </h2>
# test_dataset is divided into train, validation and test data sets and the lengths are printed to check the subset sizes

# In[145]:


# Split the data into training (40%), validation (40%), and testing (30%) sets
train_size = int(len(test_dataset) * 0.4)
val_size = int(len(test_dataset) * 0.4)
test_size = int(len(test_dataset) * 0.3)

train_data = test_dataset.iloc[:train_size]
val_data = test_dataset.iloc[train_size:train_size+val_size]
test_data = test_dataset.iloc[train_size+val_size:]
