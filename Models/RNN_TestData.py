...
# Reading the CSV files that belong to test group (EyeT_group_dataset_III_*.csv) from the dataset directory, appending them all and saving them as test_csv.csv. Here we make sure to check if a file named test_csv.csv, exists in the directory before creating a new file.
...
# Identifying all the unique "Participant name" values in the test_csv dataframe. Which returns the names of the paricipants in the test group.

# In[10]:


unique_participants_intest = test_csv["Participant name"].unique()
print(unique_participants_intest)
...
# <h1> Test data set </h1>
# 
# Invoking the feature_selection() function for the test_csv (test group)
...
# Invoking the add_empathy_scores() function for the test_csv (test group dataset) and saving the output as test_dataset
...
filename = 'test_dataset_updated.csv'
test_dataset.to_csv(filename, index=False)


# Check for the presence of NaN values in the test_dataset
...
start_time = 23419315     
end_time = 24019315
filtered_dataset = test_dataset[(test_dataset['Recording timestamp'] >= start_time) & (test_dataset['Recording timestamp'] <= end_time)]
...
timestamps = test_dataset['Recording timestamp']
GazeEventDuration = test_dataset['Gaze event duration']
...
train_size = int(len(test_dataset) * 0.8)
train_data = test_dataset.iloc[:train_size]
test_data = test_dataset.iloc[train_size:]
